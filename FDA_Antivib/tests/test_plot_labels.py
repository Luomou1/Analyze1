...
def test_high2g_profile_titles_and_ylabels_are_chinese(qtbot) -> None:
    ...
    assert window._single_profile_axes[("high2g_final", "x")].get_ylabel() == "相位差 (cycles)"
...
def test_high2g_profile_window_can_jump_to_typed_pixel_without_wheel_changes(qtbot) -> None:
...
def test_pixel_popup_ylabels_are_chinese_and_keep_units(qtbot) -> None:
    ...
    assert window.phase_unwrapped_axes.get_title() == "有效带展开与直线拟合：无有效点"
...
def test_local_unwrap_phase_profile_uses_actual_fit_range(qtbot) -> None:
    window = PixelAnalysisWindow()
    qtbot.addWidget(window)
    ...
    np.testing.assert_allclose(window.phase_unwrapped_axes.get_xlim(), (0.8, 3.2))
