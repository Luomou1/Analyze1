...
def test_surface_invalid_pixels_are_holes_not_minimum_heights():
    import numpy as np
    from app.plotting.preview import build_normalized_surface_grid
    data=np.arange(25,dtype=float).reshape(5,5)+5000
    data[2,2]=np.nan
    grid,values,_,_,_=build_normalized_surface_grid(data)
    assert np.isnan(values[2,2])
    assert grid.point_data['vtkGhostType'][12] != 0
    np.testing.assert_array_equal(data[np.isfinite(data)],values[np.isfinite(data)])
