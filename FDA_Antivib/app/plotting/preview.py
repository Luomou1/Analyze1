...
def build_normalized_surface_grid(
    ...
    if max_points is None:
        ...
        y_coords = np.arange(reduced.shape[0], dtype=np.float64)
        x_coords = np.arange(reduced.shape[1], dtype=np.float64)
    else:
        reduced, x_coords, y_coords = downsample_for_preview(np.asarray(data), max_points=max_points)
    resolved_z_bounds = _combined_finite_bounds(data) if z_bounds is None else _ensure_nonzero_limits(*z_bounds)
    z_lower, z_upper = resolved_z_bounds
    # 先用双精度扣除基准高度，避免大偏置下的纳米起伏被 float32 吞掉。
    display_values = reduced.astype(np.float64)
    invalid = ~np.isfinite(display_values)
    # 无效点只使用有限占位坐标保证VTK网格稳定；下面隐藏这些点及相邻单元。
    # 不把孔洞作为最低高度参与表面着色或连面。
    geometry_values = np.where(invalid, z_lower, display_values)
    z_normalized = np.clip((geometry_values - z_lower) / (z_upper - z_lower), 0.0, 1.0)
    ...
    if invalid.any():
        grid.hide_points(invalid.ravel(order="F"))
        # 平滑着色会提取 PolyData；仅隐藏点不能阻止提取器跨孔洞连面。
        invalid_cells = invalid[:-1, :-1] | invalid[1:, :-1] | invalid[:-1, 1:] | invalid[1:, 1:]
        if invalid_cells.size:
            grid.hide_cells(invalid_cells.ravel(order="F"), inplace=True)
...
class HeatmapCanvas(_InteractiveCanvas):
    def __init__(self, *, surface_style: bool = False) -> None:
        super().__init__(projection=None)
        self._surface_style = surface_style
    ...
    def draw_map(self, data: np.ndarray, title: str) -> None:
        ...
        self.axes.clear()
        color_limits = surface_color_limits(self._data) if self._surface_style else (None, None)
        image = self.axes.imshow(
            self._data,
            cmap=SURFACE_CMAP if self._surface_style else "viridis",
            interpolation="nearest" if self._surface_style else None,
            vmin=color_limits[0],
            vmax=color_limits[1],
            origin="lower",
...
class SpectrumCanvas(_InteractiveCanvas):
    ...
    def draw_spectrum(self, k_axis: np.ndarray, spectrum: np.ndarray, peak_index: int, title: str, k0_value: float | None = None) -> None:
        ...
        peak_k = float(self._k_axis[peak_index]) if k0_value is None else float(k0_value)
        peak_value = float(np.interp(peak_k,self._k_axis,self._spectrum))
...
class _PyVistaPreview(QWidget):
    ...
    def __init__(self) -> None:
        ...
        self._surface_jobs = SurfaceJobs(self, self._emit_info)
    ...
    def _update_surface_display(self) -> None:
        if self._shutdown:
            return
        self.surface_controls.apply(self.plotter, self._surface_shape)
    ...
    def _prepare_scene(self, title: str) -> None:
        """清空并设置统一的浅色技术预览场景。"""
        self.plotter.clear()
        configure_surface_scene(self.plotter)
        self.plotter.add_text(title, position="upper_left", font_size=10, color="#e2e8f0")
...
class SurfaceCanvas(_PyVistaPreview):
    ...
    def draw_surface(self, data: np.ndarray, title: str, *, _prepared=None) -> None:
        """将高度矩阵渲染为 PyVista 三维表面预览。"""
        if self._shutdown:
            return
        if _prepared is None and data.size >= 65536:
            snapshot = np.array(data, copy=True)
            self._emit_info("正在后台准备全分辨率三维表面…")
            self._surface_jobs.submit(lambda: prepare_surface(snapshot),
                                     lambda prepared: self.draw_surface(snapshot, title, _prepared=prepared))
            return
        if _prepared is None:
            self._surface_jobs.cancel()
        prepared = _prepared if _prepared is not None else prepare_surface(data)
        ...
        try:
            ...
            if mesh.n_points:
                ...
                self.plotter.scalar_bar.SetBarRatio(0.12)
            else:
                self.plotter.add_text("No valid surface cells", position="lower_left", color="white")
...
class ComparisonCanvas(_PyVistaPreview):
    ...
    def draw_comparison(self, height_map: np.ndarray, height_map_prime: np.ndarray, title: str, *, _prepared=None) -> None:
        """将 h 与 h_prime 以半透明 PyVista 表面叠加预览。"""
        if self._shutdown:
            return
        if _prepared is None and max(height_map.size, height_map_prime.size) >= 65536:
            left, right = np.array(height_map, copy=True), np.array(height_map_prime, copy=True)
            def prepare_pair():
                bounds = _combined_finite_bounds(left, right)
                return prepare_surface(left, bounds), prepare_surface(right, bounds)
            self._emit_info("正在后台准备全分辨率对比表面…")
            self._surface_jobs.submit(prepare_pair, lambda pair: self.draw_comparison(left, right, title, _prepared=pair))
            return
        if _prepared is None:
            self._surface_jobs.cancel()
        z_bounds = _combined_finite_bounds(height_map, height_map_prime)
        self._surface_shape = height_map.shape
        left, right = _prepared if _prepared is not None else (
            prepare_surface(height_map, z_bounds), prepare_surface(height_map_prime, z_bounds))
        h_values, hp_values, x_coords, y_coords = left.values, right.values, left.x, left.y
        self.plotter.suppress_rendering = True
        try:
            self._prepare_scene(title)
            h_mesh = left.mesh
            ...
            if h_mesh.n_points or hp_mesh.n_points:
