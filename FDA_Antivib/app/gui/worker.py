...
from app.reconstruction.engine import analyze, preview_spectrum as estimate_global_k0
...
class AnalysisWorker(QObject):
    ...
    def _emit_analysis_progress(self, percent: int) -> None:
        ...
        底层 `reconstruction.engine.analyze()` 的回调仍然使用 0~100。
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if sample_positions is not None:
                raise ValueError("Normal/High/High 2G 当前使用输入步长的均匀扫描；请关闭旧非均匀采样选项。")
            analysis_method = self.params.analysis_method
            ...
            result = analyze(
                cube=cube, step_um=self.params.step_size, mode=analysis_method,
                start_um=self.params.start_height+(active_range.start_frame-1)*self.params.step_size,
                maximum=4095.0 if self.params.image_intensity_mode=="mono12_uint16" else 255.0,
...
class GlobalK0Worker(QObject):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if sample_positions is not None:
                self.log.emit(f"公共 K0 已加载 {sample_positions.shape[0]} 个 scan_log 非均匀采样位置。")

            self.progress.emit(55)
            self.log.emit("正在计算全部有效像素的公共 K0...")
            result = estimate_global_k0(
                intensity_data=cube,
                step_size=self.step_size,
                maximum_scan_value=4095.0 if self.image_intensity_mode=="mono12_uint16" else 255.0,
                unwrap_method=self.unwrap_method,
                window_size=self.window_size,
                progress=lambda percent: self.progress.emit(55+int(percent*.45)),
