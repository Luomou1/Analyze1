...
def _height_limits(data: np.ndarray) -> tuple[float, float]:
    """围绕主体高度对称留白，保留全分辨率极值，不改变测量数据。"""
    values = data[np.isfinite(data)]
    center = float(np.median(values))
    radius = max(center - float(values.min()), float(values.max()) - center)
    # 上下各留 10% 余量；完全平坦时使用 1 nm 的可见跨度。
    half_span = max(radius * 1.1, 0.5)
    ...
    def draw_profile(
        ...
        *,
        ...
        if not np.any(np.isfinite(profile)):
            self.show_message("此轮廓没有有效高度点。")
            return
        mean = float(np.nanmean(profile))
        pv = float(np.nanmax(profile) - np.nanmin(profile))
        std = float(np.nanstd(profile))
        rms = float(np.sqrt(np.nanmean(profile ** 2)))
        maximum_index = int(np.nanargmax(profile))
...
class StepSelectionCanvas(FigureCanvasQTAgg):
    ...
    def _on_click(self, event) -> None:
        ...
        if not np.isfinite(self._data[row, col]):
            return
...
class _PlaneAnalysisWorker(QObject):
    ...
    def __init__(self, path: str, conversion_factor: float, options: SurfaceOptions) -> None:
        super().__init__()
        self._path = path
    ...
    def run(self) -> None:
        try:
            ...
            self.finished.emit(analyze_plane(matrix, options=self._options))
        except Exception as exc:  # pragma: no cover - failure path is exercised through GUI state.
            self.failed.emit(str(exc))


class _StepAnalysisWorker(QObject):
    """台阶分析后台任务，执行参数快照指定的表面处理。"""

    finished = Signal(object)
    failed = Signal(str)

    def __init__(self, matrix: np.ndarray, points: tuple[tuple[int, int], ...], options: SurfaceOptions) -> None:
        super().__init__()
    ...
    def run(self) -> None:
        try:
            self.finished.emit(analyze_step(self._matrix, points=self._points, options=self._options))
        except Exception as exc:  # pragma: no cover - failure path is exercised through GUI state.
            self.failed.emit(str(exc))
...
class PlaneAnalysisPage(QFrame):
    ...
    def _build_ui(self) -> None:
        ...
        self.surface_controls = SurfaceControls()
        form.addRow(self.surface_controls)
        ...
        self.export_button = QPushButton("导出处理后高度数据")
        self.export_button.setAccessibleName("导出平面分析高度数据")
        self.export_button.setToolTip("导出完整高度矩阵（nm），缺失点保留 NaN。")
        self.export_button.setEnabled(False)
        self.export_button.clicked.connect(self._export_height_data)
        ...
        self.surface_controls.changed.connect(self._invalidate_parameters)
        self.file_edit.textChanged.connect(self._invalidate_parameters)
        self.conversion_factor.valueChanged.connect(self._invalidate_parameters)
    ...
    def _invalidate_parameters(self) -> None:
        ...
        self.metrics.clear()
        self.status_label.setText("输入或参数已改变，请重新分析。")

    def _export_height_data(self) -> None:
        """保存最终分析矩阵，保持行列、纳米单位和无效点不变。"""
        if self._result is None or self._analysis_thread is not None:
            return
        source = Path(self.file_edit.text().strip())
        suggested = source.with_name(source.stem + "_processed_nm.txt")
        path, _ = QFileDialog.getSaveFileName(
            self, "导出平面分析高度数据（nm）", str(suggested), "Text Files (*.txt)"
        )
        if not path:
            return
        try:
            np.savetxt(path, self._result.processed, fmt="%.18e", delimiter="\t")
        except OSError as exc:
            QMessageBox.critical(self, "导出失败", f"无法保存高度数据：{exc}")
            return
        self.status_label.setText(f"已导出处理后高度数据（nm）：{path}")
    ...
    def _run_analysis(self) -> None:
        if self._analysis_thread is not None:
            return
        try:
            options = self.surface_controls.options()
        except ValueError as exc:
            self._handle_analysis_failed(str(exc))
            return
        ...
        self.conversion_factor.setEnabled(False)
    ...
    def _handle_analysis_finished(self, result: object) -> None:
        ...
        if self._result is None:
            ...
            return
        self.surface_controls.show_resolved_options(self._result.options)
        self._rendered_tabs.clear()
    ...
    def _finalize_analysis_thread(self) -> None:
        ...
        self.conversion_factor.setEnabled(True)
        ...
        self.metrics.setPlainText(
            "\n".join(
                (
                    ...
                    f"Sq：{stats['sq']:.6f} nm",
                    f"PV：{stats['pv']:.6f} nm",
                    f"有效点：{int(stats['valid_count'])}（{stats['valid_ratio']:.2%}）",
                    f"排除尖峰点：{self._result.outlier_count}",
                    'Sa/Sq 相对处理后零基准；未自动减均值，不代表完整 ISO 25178 评价。',
                    f"拟合参数：{coefficients}",
                )
            )
        )
...
class StepAnalysisPage(QFrame):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._region_report: StepRegionReport | None = None
        self._rendered_tabs: set[int] = set()
        self._analysis_thread: QThread | None = None
    ...
    def _build_ui(self) -> None:
        ...
        layout.setContentsMargins(12, 12, 12, 12)
        layout.setSpacing(10)
        layout.addWidget(SectionHeader("台阶分析", "选择高度文件，按需处理后框选两个平台测量。"))
        ...
        self.surface_controls = SurfaceControls(step=True)
        form.addRow(self.surface_controls)
        self.region_reference = QComboBox()
        self.region_reference.addItem('各平台均值面', 'mean')
        self.region_reference.addItem('处理后共同零基准', 'zero')
        self.region_reference.addItem('各平台独立拟合平面', 'plane')
        self.region_reference.setToolTip('仅改变平台 Sa/Sq/PV 的参考面；台阶高度始终使用处理后区域均值差。')
        self.region_reference.currentIndexChanged.connect(self._reference_changed)
        form.addRow('平台指标参考面', self.region_reference)
        notice = QLabel('测高请避开边缘与补洞点；全域去形状和滤波可能改变高度差。平台指标参考面不改变测高。')
        ...
        self.load_button = QPushButton("载入高度数据")
        ...
        self.export_button = QPushButton('导出台阶数据与报告')
        self.export_button.setAccessibleName('导出台阶数据与报告')
        self.export_button.setEnabled(False)
        ...
        self.surface_controls.changed.connect(self._processing_changed)
    ...
    def _load_for_selection(self) -> None:
        ...
        try:
            ...
            self.metrics.clear()
            self._show_result_placeholders("开始分析后显示结果。")
            self._processing_changed()
            if self.surface_controls.remove.currentData() != "Three Points":
                self.selection_canvas._mode = "idle"
                self.selection_canvas._draw_height_map("已载入高度，可直接开始分析")
                self.status_label.setText("高度数据已载入，可直接分析；三点调平为可选项。")
    ...
    def _processing_changed(self) -> None:
        ...
        self.metrics.clear()
        self.reset_regions_button.setEnabled(False)
        needs_points = self.surface_controls.remove.currentData() == 'Three Points'
        if needs_points and self._matrix is not None and len(self.selection_canvas.points) < 3:
            self.selection_canvas._mode = "points"
        self.reset_points_button.setEnabled(needs_points and self._matrix is not None)
        self.analyze_button.setEnabled(self._matrix is not None and
                                      (not needs_points or len(self.selection_canvas.points) == 3))
    ...
    def _run_analysis(self) -> None:
        if self._matrix is None or self._analysis_thread is not None or (self.surface_controls.remove.currentData() == 'Three Points' and len(self.selection_canvas.points) != 3):
            return
        try:
            options = self.surface_controls.options()
        except ValueError as exc:
        ...
        self.selection_canvas.setEnabled(False)
        self.reset_points_button.setEnabled(False)
        self.reset_regions_button.setEnabled(False)
        self.surface_controls.setEnabled(False)
        self.file_edit.setEnabled(False)
        self.choose_button.setEnabled(False)
    ...
    def _handle_analysis_failed(self, message: str) -> None:
        ...
        self.export_button.setEnabled(False)
        self.metrics.clear()
        hint = ('请检查三个同层且不共线的有效参考点及处理参数。'
                if self.surface_controls.remove.currentData() == 'Three Points'
                else '请检查有效数据范围、裁剪和滤波参数后重试。')
        self.status_label.setText(f"分析失败：{message} {hint}")
        QMessageBox.critical(self, "台阶分析失败", f"{message}\n\n{hint}")

    def _finalize_analysis_thread(self) -> None:
        self.load_button.setEnabled(True)
        self.selection_canvas.setEnabled(True)
        self.reset_points_button.setEnabled(self.surface_controls.remove.currentData() == "Three Points")
        self.surface_controls.setEnabled(True)
        self.file_edit.setEnabled(True)
        self.choose_button.setEnabled(True)
        self.conversion_factor.setEnabled(True)
        self._analysis_thread = None
        self._analysis_worker = None
        self.analyze_button.setEnabled(self._matrix is not None and (self.surface_controls.remove.currentData() != 'Three Points' or len(self.selection_canvas.points) == 3))
    ...
    def _on_regions_changed(self, regions: tuple[tuple[int, int, int, int], ...]) -> None:
        ...
        try:
            ...
            self._draw_profile()
            if self._region_report is not None:
                self.status_label.setText("两个区域已选择，台阶高度差和平台指标已计算。")
            self.tabs.setCurrentIndex(2)
        except Exception as exc:
    ...
    def _reference_changed(self) -> None:
        self._region_report = None
        if self._result is not None and self._measurement is not None:
            try:
                self._region_report = evaluate_regions(self._result.processed, self._measurement,
                                                       self.region_reference.currentData())
            except ValueError as exc:
                self.status_label.setText(f'平台指标无法计算：{exc}')
        self._update_metrics()

    def _export_report(self) -> None:
        if self._result is None or self._analysis_thread is not None:
            return
        if self._measurement is not None and self._region_report is None:
            QMessageBox.critical(self, '平台指标未完成', '当前区域无法建立所选参考面，请调整参考面或重新框选。')
            return
        path, _ = QFileDialog.getSaveFileName(self, '导出台阶数据与报告', 'step_report.npz', 'NumPy archive (*.npz)')
        if not path:
            return
        try:
            export_step_report(path, self._result, self._region_report)
        except (OSError, ValueError) as exc:
            QMessageBox.critical(self, '导出失败', str(exc))
            return
        self.status_label.setText(f'已导出高度矩阵、有效掩码与参数/区域报告：{path}')
    ...
    def _update_metrics(self) -> None:
        ...
        lines = [
            f"文件：{Path(self.file_edit.text()).name}",
            f"形状去除：{self.surface_controls.remove.currentText()}",
            ...
            f"排除尖峰点：{self._result.noise_count}",
        ]
        ...
        stats = surface_stats(self._result.processed)
        if self._result.options is not None and self._result.options.remove == 'Zernike Fringe':
            lines.append('Fringe 系数（nm，非 RMS 归一）：'+', '.join(
                f'Z{index}={value:.6f}' for index, value in enumerate(self._result.coefficients, 1)))
        if self._region_report is not None:
            lines.append(f'平台指标参考面：{self.region_reference.currentText()}（不改变台阶高度差）')
            for name, region in (('平台一', self._region_report.region_one), ('平台二', self._region_report.region_two)):
                lines.extend([f'{name} Sa：{region.sa:.6f} nm', f'{name} Sq：{region.sq:.6f} nm',
                              f'{name} PV：{region.pv:.6f} nm',
                              f'{name}有效点：{region.valid_count}（{region.valid_ratio:.2%}）'])
        elif self._measurement is not None:
            lines.append('平台指标未生成：当前参考面无法拟合，请调整参考面或框选区域。')
        lines.extend(measurement_notes(self._result.options))
        lines.extend([f"全评价域 Sa：{stats['sa']:.6f} nm", f"全评价域 Sq：{stats['sq']:.6f} nm", f"全评价域 PV：{stats['pv']:.6f} nm", f"有效点比例：{stats['valid_ratio']:.2%}"])
        self.metrics.setPlainText("\n".join(lines))
