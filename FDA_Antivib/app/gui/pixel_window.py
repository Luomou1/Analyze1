...
from PySide6.QtWidgets import QLabel, QMainWindow, QVBoxLayout, QWidget, QHBoxLayout, QSpinBox
...
class PixelAnalysisWindow(QMainWindow):
    ...
    coordinates_changed = Signal(str, int, int)

    def __init__(self, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self.setWindowTitle("\u50cf\u7d20\u5206\u6790")
        self.setWindowFlag(Qt.Tool, True)
        self._layer_name = "h_prime"
        self._build_ui()
    ...
    def _build_ui(self) -> None:
        ...
        self.x_coordinate = QSpinBox()
        self.y_coordinate = QSpinBox()
        for label, control in (("X（列，从0开始）", self.x_coordinate), ("Y（行，从0开始）", self.y_coordinate)):
            coordinates.addWidget(QLabel(label))
            control.setRange(0, 0)
            control.setKeyboardTracking(False)
            control.setEnabled(False)
            coordinates.addWidget(control)
            control.valueChanged.connect(self._request_coordinates)
    ...
    def set_image_shape(self, height: int, width: int) -> None:
        """坐标范围随当前数据更新，设置时不触发重新分析。"""
        for control, size in ((self.x_coordinate, width), (self.y_coordinate, height)):
            control.blockSignals(True)
            control.setRange(0, max(0, size - 1))
            control.setEnabled(size > 0)
            control.blockSignals(False)

    def _request_coordinates(self) -> None:
        self.coordinates_changed.emit(self._layer_name, self.x_coordinate.value(), self.y_coordinate.value())

    def update_analysis(self, payload: dict, layer_name: str, fitting_method: str, unwrap_method: str) -> None:
        """用新像素分析结果刷新整套图形与标题说明。"""
        self._layer_name = layer_name
        for control, key in ((self.x_coordinate, "x"), (self.y_coordinate, "y")):
            control.blockSignals(True)
            control.setValue(int(payload[key]))
            control.blockSignals(False)
        ...
        self.raw_axes.set_xlim(float(payload["signal_x"][0]), float(payload["signal_x"][-1]))
        ...
        self.dc_axes.set_xlim(float(payload["signal_x"][0]), float(payload["signal_x"][-1]))
        ...
        self.amp_axes.set_title("振幅频谱（去 DC）", fontproperties=FONT_PROP)
        ...
        if "phase_original_y" in payload:
            self.phase_raw_axes.plot(payload["k_x"], np.asarray(payload["phase_original_y"])/TAU,
                                     color="#9a9a9a", linewidth=.8, label="原扫描参考")
        self.phase_raw_axes.plot(payload["k_x"], phase_raw_cycles, color="#9467bd", linewidth=1.2,
                                 label="拟合使用的中心化参考")
        fit_band=np.asarray(payload.get("fit_k_x", []))
        if fit_band.size:
            self.phase_raw_axes.axvspan(fit_band[0],fit_band[-1],alpha=.12,color="orange",label="实际拟合频带")
        self.phase_raw_axes.legend(prop=FONT_PROP,fontsize=8,loc="best")
        self.phase_raw_axes.set_title("全频段包裹相位（非全频段拟合）", fontproperties=FONT_PROP)
        ...
        if unwrap_method == "global":
            ...
            if fit_k_x is not None and fit_phase_y is not None and len(fit_k_x) >= 2:
                ...
                self.phase_unwrapped_axes.text(
                    0.60,
                    0.12,
                    f"加权拟合斜率 = {slope:.6f} cycles/(rad/um)",
        ...
        fit_points=np.asarray(payload.get("fit_k_x", []))
        detail=(f"有效带展开与直线拟合：{fit_points.size} 点，k={fit_points[0]:.3f}–{fit_points[-1]:.3f} rad/um"
                if fit_points.size else "有效带展开与直线拟合：无有效点")
        self.phase_unwrapped_axes.set_title(detail, fontproperties=FONT_PROP)
        ...
        valid_k = np.asarray(payload.get("fit_k_x", []))
        if valid_k.size > 1:
            margin = (valid_k[-1] - valid_k[0]) * .1
            self.phase_unwrapped_axes.set_xlim(valid_k[0]-margin, valid_k[-1]+margin)
        else:
            _set_data_xlim_from_zero(self.phase_unwrapped_axes, payload["k_x"])
            self.phase_unwrapped_axes.text(.5, .5, "当前像素无有效拟合频段", ha="center", transform=self.phase_unwrapped_axes.transAxes)
        self.toolbar.update()
