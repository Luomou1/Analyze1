...
class MainWindow(QMainWindow):
    ...
    def _build_log_page(self) -> QWidget:
        ...
        hint = QLabel("记录公共 K0、分析、结果刷新和导出过程。")
    ...
    def _build_step_rail(self) -> QWidget:
        ...
        for row, (label, value) in enumerate(
            (
                ...
                ("公共 K0 处背景幅值", self.auto_k0_prominence_label),
                ("窗函数", self.auto_k0_window_label),
                ("补零", self.auto_k0_pad_label),
                ("有效像素数", self.auto_k0_candidates_label),
            )
        ):
    ...
    def _build_algorithm_parameter_page(self) -> QWidget:
        ...
        form.addRow("公共 K0（rad/μm）", self.fixed_k0)

        self.window_size = QSpinBox()
        self.window_size.setRange(1, 200)
        self.window_size.setValue(3)
        form.addRow("左右各取频点数", self.window_size)
        ...
        self.unwrap.addItem("EXE连续频段展开", "exe")
        self.unwrap.addItem("unwrap1", "unwrap1")
        self.unwrap.setCurrentIndex(self.unwrap.findData("exe"))
        form.addRow("解包裹", self.unwrap)

        self.analysis_method = QComboBox()
        self.analysis_method.addItem("Normal", "normal")
    ...
    def _build_run_parameter_page(self) -> QWidget:
        ...
        k0_group, k0_layout = self._build_section("公共 K0 摘要")
        k0_form = QFormLayout()
        self.auto_k0_value_label = QLabel("-")
        self.auto_k0_prominence_label = QLabel("-")
        self.auto_k0_window_label = QLabel("-")
        ...
        k0_form.addRow("公共 K0 处背景幅值", self.auto_k0_prominence_label)
        k0_form.addRow("窗函数", self.auto_k0_window_label)
        k0_form.addRow("补零", self.auto_k0_pad_label)
        k0_form.addRow("有效像素数", self.auto_k0_candidates_label)
    ...
    def _requires_scan_log(self) -> bool:
        ...
        if not hasattr(self, "sampling_mode") or not hasattr(self, "analysis_method"):
            return False
    ...
    def _update_run_controls(self) -> None:
        ...
        if running:
            ...
            hint = "进入“算法配置”确认参数后，才启用公共 K0 和分析。"
    ...
    def _build_left_panel_redesign(self) -> QWidget:
        ...
        self._manual_common_k0 = False
        self.fixed_k0.valueChanged.connect(self._mark_common_k0_manual)
        algorithm_form.addRow("公共 K0（rad/μm）", self.fixed_k0)

        self.window_size = QSpinBox()
        self.window_size.setRange(1, 200)
        self.window_size.setValue(3)
        algorithm_form.addRow("左右各取频点数", self.window_size)
    ...
    def _build_center_panel(self) -> QWidget:
        ...
        主结果对三种重建模式共用；high2g 模式下额外显示中间诊断图层。
        """
        panel = QFrame()
        ...
        self._high2g_diagnostics_ready = False
        self.high2g_diagnostics_button = QPushButton("显示额外诊断图")
        self.high2g_diagnostics_button.clicked.connect(self._toggle_high2g_diagnostics)
        title_row.addWidget(self.high2g_diagnostics_button)
    ...
    def _build_empty_results_page(self) -> QWidget:
        ...
        description = QLabel("完成数据与扫描、算法配置后，在右侧运行公共 K0 或正式分析。")
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        # 这些标签继续保留为状态字段，避免公共 K0 完成回调访问空属性，
    ...
    def _build_plot_tab(self, canvas, key: str) -> QWidget:
        ...
        elif key in {"phi0", "theta", "high2g_raw", "high2g_final", "fringe_order", "final_height_phase"}:
    ...
    def _update_sampling_mode_ui(self) -> None:
        ...
        self.sampling_mode.setCurrentIndex(0)
        self.sampling_mode.setEnabled(False)
    ...
    def _is_diagnostic_mode(self) -> bool:
        """当前选择 High 或 High 2G 工作流时显示诊断摘要页。"""
        return str(self.analysis_method.currentData()).strip().lower() != "normal"
    ...
    def _refresh_result_tabs(self) -> None:
        ...
        self.tabs.addTab(self._plot_tabs["k0"], "公共 K0 频谱")
        if self._is_high2g_mode() and self._high2g_diagnostics_visible:
            ...
            self.tabs.addTab(self._plot_tabs["high2g_raw"], "Disconnected high2g gap")
            self.tabs.addTab(self._plot_tabs["high2g_final"], "Final high2g gap")
            self.tabs.addTab(self._plot_tabs["fringe_order"], "Fringe Order Map")
            self.tabs.addTab(self._plot_tabs["confidence"], "置信度图")
        if self._is_diagnostic_mode() and (not self._is_high2g_mode() or self._high2g_diagnostics_visible) and self._diagnostic_tab is not None:
    ...
    def _update_analysis_mode_ui(self) -> None:
        """刷新当前工作流对应的结果页签。"""
        for name, label, data in (("fitting", "MetroPro 功率加权直线", "weighted"),
                                  ("analysis_window_name", "实际帧数原始矩形窗", "none"),
                                  ("analysis_zero_padding_mode", "按数据帧数（不补零）", "none")):
    ...
    def _cached_active_range_for_current_source(self) -> tuple[int, int] | None:
        """若公共 K0 缓存仍匹配当前数据源，则返回已确认的有效帧范围。"""
    ...
    def _refresh_zero_padding_labels(self, frame_count: int | None) -> None:
        ...
        if hasattr(self, "analysis_zero_padding_mode"):
            self.analysis_zero_padding_mode.setEnabled(False)
    ...
    def _build_params(self) -> AnalysisParams:
        ...
        workflow = str(self.analysis_method.currentData())
        fixed_k0_value = float(self.fixed_k0.value())
        fixed_k0 = fixed_k0_value if self._manual_common_k0 and fixed_k0_value > 0.0 else None
    ...
    def closeEvent(self, event) -> None:
        ...
        for page_name in ('plane_analysis_page', 'step_analysis_page'):
            ...
            if page is not None and page._analysis_thread is not None:
    ...
    def _toggle_high2g_diagnostics(self) -> None:
        ...
        if not self._high2g_diagnostics_visible:
            if not self._high2g_diagnostics_ready:
                ...
                self._plot_tabs["theta"] = self._build_plot_tab(self.theta_canvas, "theta")
                self.final_height_phase_canvas = HeatmapCanvas()
                self._plot_tabs["final_height_phase"] = self._build_plot_tab(self.final_height_phase_canvas, "final_height_phase")
                self.merit_canvas = HeatmapCanvas()
                self._plot_tabs["merit"] = self._build_plot_tab(self.merit_canvas, "merit")
                self.high2g_raw_canvas = HeatmapCanvas()
                self._plot_tabs["high2g_raw"] = self._build_plot_tab(self.high2g_raw_canvas, "high2g_raw")
                self.high2g_final_canvas = HeatmapCanvas()
                self._plot_tabs["high2g_final"] = self._build_plot_tab(self.high2g_final_canvas, "high2g_final")
                self.fringe_order_canvas = HeatmapCanvas()
                self._plot_tabs["fringe_order"] = self._build_plot_tab(self.fringe_order_canvas, "fringe_order")
                self.confidence_canvas = HeatmapCanvas()
                self._plot_tabs["confidence"] = self._build_plot_tab(self.confidence_canvas, "confidence")
                self._high2g_diagnostics_ready = True
            analysis_result = self._result
            ...
            self._draw_optional_diagnostic_map(
                analysis_result,
                "high2g_raw",
                self.high2g_raw_canvas,
                "Disconnected high2g gap (cycles)",
            )
            self._draw_optional_diagnostic_map(
                analysis_result,
                "high2g_final",
                self.high2g_final_canvas,
                "Final high2g gap (cycles)",
            )
            self._draw_optional_diagnostic_map(
                analysis_result,
                "fringe_order_map",
        ...
        self._high2g_diagnostics_visible = not self._high2g_diagnostics_visible
        self.high2g_diagnostics_button.setText("隐藏额外诊断图" if self._high2g_diagnostics_visible else "显示额外诊断图")
        self._refresh_result_tabs()
        if self._high2g_diagnostics_visible:
            self.tabs.setCurrentWidget(self._plot_tabs["theta"])
    ...
    def _handle_auto_k0_finished(self, result: dict) -> None:
        ...
        # 主动请求自动计算成功后，采用新结果；计算失败不改动手动值。
        self._manual_common_k0 = False
        self.fixed_k0.setToolTip("自动公共K0；手动修改后使用输入值，再次自动计算成功后恢复自动。")
        self._show_automatic_common_k0(float(result["k0_value"]))
        ...
        self.k0_canvas.draw_spectrum(result["k_axis"], result["spectrum"], int(result["peak_index"]), "公共 K0（有效像素插值峰均值）", k0_value=float(result["k0_value"]))
        self.result_content_stack.setCurrentWidget(self.tabs)
        self.tabs.setCurrentWidget(self.k0_tab)
        self.status_label.setText("公共 K0 已就绪")
        self._set_preview_stage(2)
        self._refresh_step_states()
        self._append_log(f"公共 K0 = {float(result['k0_value']):.6f}")
    ...
    def _show_automatic_common_k0(self, value: float) -> None:
        ...
        if self._manual_common_k0:
            return
        self.fixed_k0.blockSignals(True)
        self.fixed_k0.setValue(value)
        self.fixed_k0.blockSignals(False)

    def _handle_pixel_coordinates(self, layer: str, x: int, y: int) -> None:
        """手工坐标与热图点击共用同一分析及刷新路径。"""
        self._handle_pixel_click(layer, {"x": float(x), "y": float(y)})
    ...
    def _handle_pixel_click(self, layer: str, payload: dict[str, float]) -> None:
        ...
        if self._pixel_window is None:
            ...
            self._pixel_window.coordinates_changed.connect(self._handle_pixel_coordinates)
        self._pixel_window.set_image_shape(*self._analysis_cube.shape[:2])
        ...
        analysis = build_pixel_analysis(
            ...
            start_height=self._analysis_params.start_height + ((self._analysis_params.active_range[0] - 1) * self._analysis_params.step_size if self._analysis_params.active_range is not None and self._analysis_params.sample_positions_um is None else 0.0),
    ...
    def _handle_profile_click(self, layer: str, payload: dict[str, float]) -> None:
        """响应 high2g 诊断图层点击，弹出剖面窗口联查相位与级次。"""
        ...
        layer_sources: dict[str, np.ndarray | None] = {
            ...
            "high2g_raw": self._result.extras.get("high2g_raw"),
            "high2g_final": self._result.extras.get("high2g_final"),
        }
        ...
        layer_key_map = {
            ...
            "high2g_raw": "high2g_raw",
            "high2g_final": "high2g_final",
        }
        ...
        # `phi0` 是结果对象主字段，不在 extras 里；这里显式补进去，便于和 high2g gap 图层联查。
    ...
    def _handle_auto_k0_failed(self, message: str) -> None:
        """处理公共 K0 失败：更新状态、弹窗并记录日志。"""
        self.status_label.setText("公共 K0 失败")
        QMessageBox.critical(self, "公共 K0 失败", message)
        self._append_log(f"错误：{message}")
